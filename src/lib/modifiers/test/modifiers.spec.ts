import type {Modifier, ClientRect} from '$core/index.js';
import type {FirstArgument, Transform} from '$utilities';

import {restrictToHorizontalAxis, restrictToVerticalAxis} from '../src/index.js';
...
describe('@dnd-kit/modifiers', () => {
	...
	it('restrictToHorizontalAxis', () => {
		...
		expect(restrictToHorizontalAxis({...defaultArguments, transform})).toStrictEqual({
			...transform,
			y: 0,
		});
	});
	...
	it('restrictToVerticalAxis', () => {
		...
		expect(restrictToVerticalAxis({...defaultArguments, transform})).toStrictEqual({
			...transform,
			x: 0,
		});
	});
