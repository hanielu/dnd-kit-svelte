...
import {ref, type Ref} from 'runed';
 ...
 * ```
 */
export function wrapSignal<T>(signal: SignalLike<T>): Ref<T> {
	const read = () => {
	...
	const proxy = new Proxy(obj, {
		get(target, property) {
			...
			if (signalCache.has(property)) {
				return signalCache.get(property)!.get.current;
			}
