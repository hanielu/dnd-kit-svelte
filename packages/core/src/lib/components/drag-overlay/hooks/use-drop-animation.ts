import type {MeasuringConfiguration} from '$core/components/dnd-context/types.js';
import type {Active, DraggableNodes, DroppableContainers, DraggableNode} from '$core/store/index.js';
...
import {scrollIntoViewIfNeeded} from '$core/utilities/scroll/index.js';
import {parseTransform} from '$core/utilities/transform/index.js';
import {type DeepRequired, CSS, getWindow, type Transform} from '@dnd-kit-svelte/utilities';
...
const defaultKeyframeResolver: KeyframeResolver = ({transform: {initial, final}}) => [
	{
...
export function useDropAnimation({
	...
	return (id, node) => {
		...
		const animation: DropAnimationFunction = typeof config === 'function' ? config : createDefaultDropAnimation(config);
...
function createDefaultDropAnimation(options: DropAnimationOptions | undefined): DropAnimationFunction {
