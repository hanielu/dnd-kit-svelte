<script lang="ts">
	...
	let {id, title, link, children}: SectionProps = $props();
</script>

<section {id} class="container-base flex|col gap-5">
	<div class="flex|col gap-4 md:(flex-row flex-s-between)">
		<h2 class="text-2xl fw-bold tracking--3%">{title}</h2>
		<div>
			<a
				href="https://github.com/hanielu/dnd-kit-svelte/blob/experimental/docs/src/lib/components/examples{link}"
				target="_blank"
			...
			>
				<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 256 256"
					...
					></svg
				>
				<span class="text-white fw-bold">View Code</span>
			</a>
		</div>
	</div>
